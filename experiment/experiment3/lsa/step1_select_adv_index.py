        ...
        for class_num in range(10):
            ...
            for sort_num in range(len(sort)):
                ...
                if class_test_buckets[temp_bucket] == 0:
                    ...
                    can_adv_bucket.append(temp_bucket)
            can_adv_index = np.array(can_adv_index)
            can_adv_bucket = np.array(can_adv_bucket)
            print(class_num)
            for temp_rate in rate:
                ...
                select_bucket = can_adv_bucket[:select_num]
                select_bucket = np.array(select_bucket)
                select_index = np.array(select_index)
                print(len(select_bucket))
                np.save(os.path.join(save_rate_path, str(class_num) + '_select_index.npy'), select_index)
                np.save(os.path.join(save_rate_path, str(class_num) + '_select_bucket.npy'), select_bucket)
